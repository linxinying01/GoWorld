...
func (h *AuthHandler) Register(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		...
		c.JSON(http.StatusBadRequest, models.NewErrorResponse(http.StatusBadRequest, "无效的请求格式"))
		return
	}
	...
	if err != nil {
		...
		c.JSON(errorToStatusCode(err), models.NewErrorResponse(http.StatusBadRequest, err.Error()))
		return
	}

	c.JSON(http.StatusCreated, models.NewSuccessResponse(authRes))
}
...
func (h *AuthHandler) Login(c *gin.Context) {
	...
	c.JSON(http.StatusOK, models.NewSuccessResponse(authRes))
}
